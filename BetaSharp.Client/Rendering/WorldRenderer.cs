 ...
 / 100.0F) * 0.2F + 0.4F) * 0.5F);
        ...
        if (var3 == 0)
        {
            if (damagePartialTime > 0.0F)
            {
                ...
                var6.disableColor();
                BlockRenderer.RenderBlockByRenderType(world, var9, new BlockPos(var2.BlockX, var2.BlockY, var2.BlockZ), var6,renderEngine, 240 + (int)(damagePartialTime * 10.0F), true);
                var6.draw();
