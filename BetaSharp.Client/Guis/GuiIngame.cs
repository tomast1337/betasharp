...
public class GuiIngame : Gui
{
    ...
    public void renderGameOverlay(float partialTicks, bool unusedFlag, int unusedA, int unusedB)
    {
        ...
        {
            if (_chatMessageList[j].UpdateCounter < 200 || chatOpen)
            {
                double d = _chatMessageList[j].UpdateCounter / 200.0D;
                d = 1.0D - d;
                d *= 10.0D;
                if (d < 0.0D)
                {
                ...
                if (alpha > 0)
                {
                    byte left = 2;
                    int y = -j * 9;
                    debugStr = _chatMessageList[j].Message;
    ...
    private void addWrappedChatMessage(string message)
    {
        ...
        while (_chatMessageList.Count > 50)
        {
