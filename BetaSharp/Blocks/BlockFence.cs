...
internal class BlockFence(int id, int texture) : Block(id, texture, Material.Wood)
{
    public override bool CanPlaceAt(CanPlaceAtContext context) =>
        context.World.Reader.GetBlockId(context.X, context.Y - 1, context.Z) == Id || (context.World.Reader.GetMaterial(context.X, context.Y - 1, context.Z).IsSolid && base.CanPlaceAt(context));
