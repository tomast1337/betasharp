...
public abstract class Entity
{
        ...
        if (checkWaterCollisions())
        {
            if (!inWater && !firstTick)
            {
                ...
                for (var3 = 0; var3 < 1.0F + width * 20.0F; ++var3)
                {
                    ...
                    world.Broadcaster.AddParticle("bubble", x + var4, var2 + 1.0F, z + var5, velocityX, velocityY - random.NextFloat() * 0.2F, velocityZ);
                }
                ...
                for (var3 = 0; var3 < 1.0F + width * 20.0F; ++var3)
                {
                    var4 = (random.NextFloat() * 2.0F - 1.0F) * width;
                    var5 = (random.NextFloat() * 2.0F - 1.0F) * width;
                    world.Broadcaster.AddParticle("splash", x + var4, var2 + 1.0F, z + var5, velocityX, velocityY, velocityZ);
                }
            }
    ...
    public bool isWet() => inWater || world.Environment.IsRainingAt(MathHelper.Floor(x), MathHelper.Floor(y), MathHelper.Floor(z));
    ...
    public virtual void setWorld(IWorldContext world) => this.world = world;
    ...
    public virtual void onCollision(Entity entity)
    {
        if (entity.passenger != this && entity.vehicle != this)
        {
            ...
            if (var6 >= 0.01F)
            {
                ...
                if (var2 > maxHorizontalImpulsePerCollision)
                {
                    var2 = maxHorizontalImpulsePerCollision;
                }
                else if (var2 < -maxHorizontalImpulsePerCollision)
                {
                    var2 = -maxHorizontalImpulsePerCollision;
                }

                if (var4 > maxHorizontalImpulsePerCollision)
                {
                    var4 = maxHorizontalImpulsePerCollision;
                }
                else if (var4 < -maxHorizontalImpulsePerCollision)
                {
                    var4 = -maxHorizontalImpulsePerCollision;
                }
    ...
    protected string? getRegistryEntry() => Type?.Id;
    ...
    {
        EntityItem var3 = new(world, x, y + var2, z, var1);
    ...
    public virtual void setPositionAndAnglesAvoidEntities(double x, double y, double z, float var7, float var8, int var9)
    {
        ...
        List<Box> var10 = world.Entities.GetEntityCollisionsScratch(this, boundingBox.Contract(1.0D / 32.0D, 0.0D, 1.0D / 32.0D));
        if (var10.Count > 0)
        {
            double var11 = 0.0D;
